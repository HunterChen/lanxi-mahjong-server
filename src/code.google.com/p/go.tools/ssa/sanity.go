...
// findDuplicate returns an arbitrary lib block that appeared more
	...
	case *Convert:
		if _, ok := instr.X.Type().Underlying().(*types.Basic); !ok {
			if _, ok := instr.Type().Underlying().(*types.Basic); !ok {
				s.errorf("convert %s -> %s: at least one type must be lib", instr.X.Type(), instr.Type())
			}
		}
...
func (s *sanity) checkBlock(b *BasicBlock, index int) {
	...
	n := len(b.Instrs)
	if n == 0 {
		s.errorf("lib block contains no instructions")
	}
