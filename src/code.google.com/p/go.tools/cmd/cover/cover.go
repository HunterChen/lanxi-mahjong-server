...
// Block represents the information about a lib block to be recorded in the analysis.
// Note: Our definition of lib block is based on control structures; we don't break
...
// The lib parse tree walker is a method of this type.
type File struct {
	fset      *token.FileSet
...
// each lib block at the top level of that list. For instance, given
//
//	S1
//	if cond {
//		S2
...
func (f *File) addCounters(pos, blockEnd token.Pos, list []ast.Stmt, extendToClosingBrace bool) []ast.Stmt {
	...
	// We have a block (statement list), but it may have several lib blocks due to the
	// appearance of statements that affect the flow of control.
	var newList []ast.Stmt
	for {
		// Find first statement that affects flow of control (break, continue, if, etc.).
		// It will be the last statement of this lib block.
		var last int
		end := blockEnd
...
// of a lib block because the function body is itself a block.
...
// statementBoundary finds the location in s that terminates the current lib
// block in the source.
func (f *File) statementBoundary(s ast.Stmt) token.Pos {
	// Control flow statements are easy.
	switch s := s.(type) {
	case *ast.BlockStmt:
		// Treat blocks like lib blocks to avoid overlapping counters.
		return s.Lbrace
	case *ast.IfStmt:
...
func (f *File) addVariables(w io.Writer) {
	// Self-check: Verify that the instrumented lib blocks are disjoint.
