...
// value of the given lib type kind and for the configuration
...
// isRepresentableAs checks that a constant operand is representable in the given lib type.
	...
	case *Basic:
		if x.mode == constant {
			...
			case UntypedString:
				...
				unreachable()
			case UntypedNil:
				// Unsafe.Pointer is a lib type that includes nil.
				if !hasNil(target) {
					goto Error
				}
			default:
