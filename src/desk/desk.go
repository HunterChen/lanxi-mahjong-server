...
func (t *Desk) skipL(seat uint32, huValue int64) {
	t.skip[seat] = huType(huValue, config.Opts().Ante) //跳过胡牌过圈设置
}
...
func (t *Desk) chow_(card uint32, value int64, seat uint32) bool {
	...
	//吃操作协议消息通知
	var card2 uint32 = algorithm.EncodeChow(c1, c2, t.discard)
...
func (t *Desk) getSkip(seat uint32, huValue int64) bool {
	if v, ok := t.skip[seat]; ok {
		return huType(huValue, config.Opts().Ante) > v // 胡牌番值大于前一个胡牌番值则可以胡牌
	}
	return true
}
